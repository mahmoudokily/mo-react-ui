...
import { DOMHelper, TooltipAttributes } from "../../shared";
  ...
  useEffect(() => {
    ...
    if (!DOMHelper.isMobile && containerEl?.current) {
      DOMHelper.addEventListener(
        containerEl?.current as HTMLElement,
        ["mouseover"],
        open
      );
      DOMHelper.addEventListener(
  ...
  const closeIfNotInRange = (e: Event) => {
    if (
      ...
      !(
        ...
        containerEl.current?.contains(e.target as HTMLElement)
      )
    ) {
      setStatus(false);
    }
    ...
    if (status) {
      setAttrs(
        TooltipHelper.getAttributes(
          ...
          $position,
          undefined,
          arrowEl?.current as HTMLElement
        )
      );
    }
  };

import { TooltipAttributes, TooltipPositionResult } from "./Helpers.Props";
...
export default class TooltipHelper {
  ...
  private static getPosition(
    ...
    switch (position) {
      ...
      case "left":
        top = !header
          ...
          : container.top +
            container.height / 2 -
            (header.height + arrowHeight);

        if (
          (diff = top + container.height + arrowHeight * 2) > global.height ||
          top < 0
        ) {
          if (diff > global.height) top -= diff;
          if (top < 0 && container.top > 0) top = 0;
        ...
        if (top < 0 || top + target.height > global.height) {
          ...
          let topOverflow;
          if (top < 0) topOverflow = Math.abs(top);
          else topOverflow = Math.abs(top + target.height - global.height);
      ...
      case "bottom":
        ...
        if ((diff = left + container?.width + arrowWidth - global?.width) >= 0)
          left -= diff;
        if (left < 0 && container.left > 0) left = 0;
        ...
        if (left < 0 || left + target?.width > global?.width) {
          ...
          let leftOverflow;
          if (left < 0) leftOverflow = Math.abs(left);
          else leftOverflow = Math.abs(left + target.width - global.width);
    ...
    return {
      ...
      arrowTransform: `translate(${arrowLeft?.toFixed(
        0
      )}px, ${arrowTop?.toFixed(0)}px)`,
      isArrowInHeader,
    };
  }
  ...
  static getAttributes(
    ...
    const headerRect: DOMRect | null = header
    ...
    for (let i in this.positionOrder[position]) {
      ...
      const _result = this.getPosition(
        targetRect,
        containerRect,
        global,
        _position,
